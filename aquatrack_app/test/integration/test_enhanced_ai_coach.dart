...
Future<void> testAIChatResponses(HttpClient client, String token) async {
  final testMessages = [
    {
      "message": "Xin chào, tôi cần động lực",
      "context": {"mood": "tired"}
    },
    {
    ...
    {
      "message": "Tôi thấy khó uống đủ nước mỗi ngày",
      "context": {"mood": "frustrated"}
    },
    {"message": "Progress check please", "context": {}},
    {
      "message": "Cảm ơn vì lời khuyên!",
      "context": {"mood": "grateful"}
    },
  ];
  ...
  for (int i = 0; i < testMessages.length; i++) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "   ✅ Test ${i + 1}: ${aiResponse['coaching_type']} response (${aiResponse['motivation_level']})");
        ...
        if (aiResponse.containsKey('response') &&
          ...
          print(
              "      - Response length: ${aiResponse['response'].length} chars");
...
Future<void> testContextAwareAI(HttpClient client, String token) async {
  ...
  for (int i = 0; i < contexts.length; i++) {
    try {
      ...
      request.headers.contentType = ContentType.json;
      request
          .write(jsonEncode({"message": baseMessage, "context": contexts[i]}));
      ...
      if (response.statusCode == 200) {
        final aiResponse = jsonDecode(data);
        print(
            "   ✅ Context ${i + 1}: ${contexts[i]['mood']}/${contexts[i]['location']} → ${aiResponse['coaching_type']}");
      } else {
...
Future<void> testUserScenarios(HttpClient client, String token) async {
  ...
  final scenarios = [
    ...
    {
      "name": "Goal Achiever",
      "message": "I consistently meet my daily water goal"
    },
  ...
  for (final scenario in scenarios) {
    try {
      ...
      if (response.statusCode == 200) {
        final aiResponse = jsonDecode(data);
        print(
            "   ✅ ${scenario['name']}: ${aiResponse['coaching_type']} coaching");
      } else {
  ...
  for (final endpoint in endpoints) {
    try {
      ...
      if (endpoint["method"] == "GET") {
        request =
            await client.getUrl(Uri.parse('$BASE_URL${endpoint["path"]}'));
      } else {
        request =
            await client.postUrl(Uri.parse('$BASE_URL${endpoint["path"]}'));
        request.headers.contentType = ContentType.json;
...
Future<void> testAnalyticsIntegration(HttpClient client, String token) async {
  ...
  final analyticsTestCases = [
    {
      "message": "How am I doing with my hydration?",
      "expected_context": "progress_inquiry"
    },
    {
      "message": "Give me personalized advice",
      "expected_context": "personalization_request"
    },
    {
      "message": "What should I focus on improving?",
      "expected_context": "improvement_advice"
    },
  ];
  ...
  for (int i = 0; i < analyticsTestCases.length; i++) {
    try {
      ...
      if (response.statusCode == 200) {
        final aiResponse = jsonDecode(data);
        print(
            "   ✅ Analytics test ${i + 1}: Response tailored (${aiResponse['response'].length} chars)");

        // Check if response shows signs of personalization
        if (aiResponse['suggestions'].length > 0 ||
            aiResponse['action_items'].length > 0) {
  ...
  for (final testCase in errorTestCases) {
    try {
      ...
      if (response.statusCode == 200) {
        final aiResponse = jsonDecode(data);
        print(
            "   ✅ ${testCase['name']}: Handled gracefully (${aiResponse['coaching_type']})");
...
Future<void> testPerformanceStress(HttpClient client, String token) async {
  ...
  if (successful >= concurrentRequests * 0.8) {
    print(
        "      - ✅ Performance: GOOD (${(successful / concurrentRequests * 100).round()}% success rate)");
  } else {
    print(
        "      - ⚠️ Performance: NEEDS IMPROVEMENT (${(successful / concurrentRequests * 100).round()}% success rate)");
  }
}
...
void printTestSummary() {
  ...
  print(
      "   - Multi-provider AI fallback (Anthropic → OpenAI → Ollama → Rules)");
