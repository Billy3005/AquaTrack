...
class CRUDConversation(CRUDBase[Conversation, MessageCreate, MessageUpdate]):
    ...
    def get_recent_messages_by_user(
        ...
        return (
            db.query(self.model)
            .filter(and_(self.model.user_id == user_id, self.model.is_deleted == False))
    ...
    def get_message_by_id(
        self, db: Session, *, user_id: str, message_id: str
    ...
    def get_conversation_context(
        self, db: Session, *, user_id: str, session_id: str, limit: int = 10
    ) -> List[Conversation]:
...
class CRUDConversationSession(
    CRUDBase[ConversationSession, ConversationSessionCreate, ConversationSessionUpdate]
):
    def create_session(
        self, db: Session, *, user_id: str, session_create: ConversationSessionCreate
    ) -> ConversationSession:
    ...
    def get_or_create_session(
        ...
        *,
        ...
        session_create = ConversationSessionCreate(
            session_id=new_session_id, title=title
        )
    ...
    def get_sessions_by_user(
        ...
        *,
        ...
        if active_only:
            query = query.filter(
                and_(self.model.is_active == True, self.model.is_archived == False)
            )

        return (
            query.order_by(desc(self.model.last_message_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def archive_session(
        ...
        session = (
            db.query(self.model)
            .filter(
                and_(self.model.user_id == user_id, self.model.session_id == session_id)
            )
            .first()
        )
