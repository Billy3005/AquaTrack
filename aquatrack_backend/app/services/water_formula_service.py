...
class WaterFormulaService:
    ...
    def calculate_daily_water(cls, profile: UserWaterProfile) -> WaterCalculationResult:
        ...
        activity_add = int(profile.weight * cls.ACTIVITY_MULTIPLIERS[profile.activity_level])

        # B2 - Job addition
        job_add = cls.JOB_ADDITIONS[profile.job_type]

        # B3 - Health conditions addition (sum of all selected)
        health_add = sum(cls.HEALTH_ADDITIONS[condition] for condition in profile.health_conditions)
        ...
        # Total calculation
        total_ml = base_ml + activity_add + job_add + health_add + veggie_add + coffee_add + alcohol_add
        ...
        has_warnings = any(condition in [HealthCondition.NEUROLOGICAL, HealthCondition.HEART]
                          for condition in profile.health_conditions)
        warning_message = None
        if has_warnings:
            warning_message = ("Thông tin này không thay thế lời khuyên y tế. "
...
def test_formula():
    ...
    print(f"Breakdown: base={result.base_ml}, activity={result.activity_add}, "
