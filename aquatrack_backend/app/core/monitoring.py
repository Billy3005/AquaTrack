        ...
        for endpoint, metrics in self.request_metrics.items():
            if metrics['count'] > 0:
                ...
                if recent_times:
                    ...
                    p95_time = sorted(recent_times)[int(len(recent_times) * 0.95)] if len(recent_times) >= 20 else max(recent_times)
                else:
                    recent_avg = avg_time
                    p95_time = avg_time
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        for metric_name, metric_data in self.system_metrics.items():
            if metric_data:
                ...
                if recent_values:
                    stats[f'{metric_name}_avg_5min'] = round(sum(recent_values) / len(recent_values), 2)
...
class HealthChecker:
    ...
    async def get_comprehensive_health() -> Dict[str, Any]:
        ...
        warning_count = sum(1 for check in health_checks.values() if check['status'] in ['warning', 'unhealthy'])
