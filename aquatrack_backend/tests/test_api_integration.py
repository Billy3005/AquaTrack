...
def test_user_api_response():
    ...
    conn = sqlite3.connect("aquatrack_water_formula.db")
    cursor = conn.cursor()
    ...
    print(
        f"  Weight-Height: {format_weight_height_display(backend_data['weight'], backend_data['height'])}"
    )
    print(
        f"  Gender-Age: {format_gender_age_display(backend_data['gender'], backend_data['age'])}"
    )
    print(
        f"  Activity Level: {format_activity_level_display(backend_data['activity_level'])}"
    )
    print(f"  Job Type: {format_job_type_display(backend_data['job_type'])}")
    print(
        f"  Coffee-Alcohol: {format_coffee_alcohol_display(backend_data['coffee_cups_per_day'], backend_data['alcohol_units_per_day'])}"
    )
