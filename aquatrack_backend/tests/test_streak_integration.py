...
def test_user_stats_api():
    ...
    try:
        ...
        print(
            "FAIL Cannot connect to backend server. Make sure it's running on http://localhost:8001"
        )
        return False
...
def test_create_intake_log():
    ...
    try:
        ...
        intake_response = requests.post(
            f"{BASE_URL}/intake/", json=intake_data, headers=headers
        )
        if intake_response.status_code == 201:
            ...
            if intake_result.get("level_progress"):
                ...
                print(
                    f"   Goal achieved today: {level_progress.get('goal_achieved_today', False)}"
                )
                print(f"   Today total: {level_progress.get('today_total_ml', 0)}ml")
                print(f"   Daily goal: {level_progress.get('daily_goal_ml', 2000)}ml")
                print(
                    f"   Current streak: {level_progress.get('current_streak', 0)} days"
                )
        else:
        ...
        if stats_response.status_code == 200:
            ...
            if after_streak != before_streak:
                ...
                print(
                    f"INFO Streak unchanged: {after_streak} (expected if goal not reached)"
                )
